...
import { formatPrice, genderLabel, roomTypeLabel } from "@housemates/shared-utils";
import { cn } from "@housemates/shared-utils";
import type { Listing } from "@housemates/shared-types";
...
export function ListingCard({ listing, showStatus = false }: { listing: Listing; showStatus?: boolean }) {
  ...
  return (
    <Link to={`/listings/${listing.id}`} className="group block rounded-2xl transition-transform duration-300 hover:-translate-y-1">
      <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-muted shadow-card transition-shadow duration-300 group-hover:shadow-elevated">
        {photo ? (
      ...
      <div className="space-y-1.5 px-0.5 pt-3">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="shrink-0 text-base font-bold text-foreground">{formatPrice(listing.price, listing.pricePeriod)}</span>
        </div>
